...
export const RECIPIENT_COLOR_STYLES = {
  ...
  blue: {
    ...
    comboxBoxItem:
      'ring-recipient-blue hover:bg-recipient-blue/15 active:bg-recipient-blue/15',
  },
...
export const useRecipientColors = (index: number, isFromRecipientSelection?: boolean) => {
  if (!isFromRecipientSelection) {
    // console.log("Index in Colors: ", index)
  
    const { getStepData } = usePDFContext()
    const step2Data = getStepData(2)
    // console.log("Step 2 Data: ", step2Data)
  
    // Get the actual color from step 2 data (signers form)
    const signers = step2Data?.signers || []
    // console.log("Signers Data: ", signers)
  
    const signer = signers[index]
    // console.log("Signer at Index: ", signer)
  
    if (signer?.color) {
      console.log("Inside condition and color is: ", signer.color)
      
      const key = AVAILABLE_RECIPIENT_COLORS[index % AVAILABLE_RECIPIENT_COLORS.length]
      return {
        ...RECIPIENT_COLOR_STYLES[key], // Use predefined Tailwind classes
        actualColor: signer.color,
        inlineStyles: {
          ringColor: signer.color,
          backgroundColor: signer.color,
          shadowColor: signer.color
        }
      }
    }
  }
...
export const getRecipientColorStyles = (index: number) => {
  ...
  return useRecipientColors(index, true)
}
