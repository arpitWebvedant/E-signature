...
export const useDocumentElement = () => {
  ...
  const getPage = (event: MouseEvent, pageSelector: string) => {
    ...
    const $page =
      ...
      target.querySelector<HTMLElement>(pageSelector)
  ...
  const isWithinPageBounds = useCallback(
    (
      ...
      if (
        event.clientY > top + height - halfMouseHeight ||
        event.clientY < top + halfMouseHeight
      ) {
        return false
      }

      if (
        event.clientX > left + width - halfMouseWidth ||
        event.clientX < left + halfMouseWidth
      ) {
        return false
      }
